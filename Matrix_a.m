...
%number1    
   en = zeros(L,1);%零矩阵
    ...
    P = zeros(L, L);
        ...
        tmp = (2/ L^2) * ul*(P-Q)*en; 
        w_lms = w_lms + 0.08 * tmp;       
    end
    
    %number 2 a=0.25 b=2 c=4
    en = zeros(L,1);
    for ii = L : LEN
    ...
    P = zeros(L, L);
        ...
        tmp = (2/ L^2)*ull*w7*(P-Q)*w7*en; 
        w_lms4 = w_lms4 + 0.065 * tmp;       
    end
            %number 5 a=2 b=2 c=4
    en = zeros(L,1);
    for ii = L : LEN
...
legend('MEE','MEE+Hampel(a=0.25)','MEE+Hampel(a=0.5)','MEE+Hampel(a=1)','MEE+Hampel(a=2)');
xlabel('iteration');
