...
for mm = 1 : 10
    ...
    for ii = 1 : 1000
        DD(ii) = wo1' * UU(:,ii) + VV(ii);%产生1*3000的行向量，也即是输出
    end
    wo2 = randn(m, 1);%初始w0为30*1
    for ii = 1001 : 2000
        DD(ii) = wo2' * UU(:,ii) + VV(ii);%产生1*3000的行向量，也即是输出    
    end
    wo3 = randn(m, 1);%初始w0为30*1
    for ii = 2001 : 3000
        DD(ii) = wo3' * UU(:,ii) + VV(ii);%产生1*3000的行向量，也即是输出    
    end
    w_lms = randn(m, 1);
    w_mee1 = w_lms;
    w_mee2 = w_lms;
